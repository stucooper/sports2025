...
the Redcliffe (suburb of Brisbane) Dolphins; commentators and media
...
games for the Sydney, Brisbane and Gold Coast teams, to promote
Aussie Rules in NSW and QLD.  AFL fans \emph{hate}
Round Zero. In 2025 Cyclone Alfred cancelled the
Queensland games and there were just two dull Sydney games.
...
In the NRL ladder a team with a week off (the ``Bye'') scores
2 competition points that round, the same as if they had played and
won. So an NRL results file finishes with the team(s) that had the bye
that week.
...
Get the results files (in sorted order) and process each one:
\begin{verbatim}
...
is a longstanding NRL bet under the ``Footy TAB'' system. For just
\$1, anyone over the age of 18 can develop a hard-to-shake-off Sports
Betting habit. Each match has the team expected to lose given a points
start to try to even things up. If that team loses, but wins once the
point start is applied, they win for the purposes of Pick the
Winners.
